...
!((t, e) => {
	...
	function L(t, e) {
		return t.actions.forEach((t) => t.vars[e] && t.vars[e](t));
	}
	function S(t) {
	...
	function X(t, e, i) {
		return t.forEach((t) => t.classList[i](e));
	}
	function $(t) {
	...
	function ea(t, e, i) {
		return (
			t.forEach(
				(t) =>
					(t.d = (function _getDOMDepth(t, e, i) {
						void 0 === i && (i = 0);
						for (
							var n = t.parentNode,
								r = 1e3 * 10 ** i * (e ? -1 : 1),
	...
	function ha(t, e) {
		t.forEach((t) => (t.a.cache.uncache = 1)), e || t.finalStates.forEach(ga);
	}
	function ja(e, i, n) {
		var r,
			a,
		...
		if (("object" != typeof i && (i = e), tt && 1 !== n))
			return (
				...
				tt._final.push(() => (e.cache.uncache = 1) && ga(e)),
				o
			);
		for (
			a = "none" === c("display"),
	...
	function Aa(t, e, r, i) {
		...
		z && (st[z] || ba(z)).forEach((e) => (Y[e] = (t) => H[t].a.props[e])),
			(H.finalStates = f = []),
			(m = function run() {
				...
				if (
					(I &&
						((e = f.map((t) => t.element)),
						L && e.forEach((t) => t.classList.remove(I))),
					za(!1),
					A
						? ((Y.scaleX = (t) => H[t].a.scaleX),
							(Y.scaleY = (t) => H[t].a.scaleY))
						: ((Y.width = (t) => H[t].a.width + "px"),
							(Y.height = (t) => H[t].a.height + "px"),
							(Y.autoRound = r.autoRound || !1)),
					(Y.x = (t) => H[t].a.x + "px"),
					(Y.y = (t) => H[t].a.y + "px"),
					(Y.rotation = (t) => H[t].a.rotation + (N ? 360 * Z(t, c[t], c) : 0)),
					(Y.skewX = (t) => H[t].a.skewX),
					(c = H.map((t) => t.t)),
					(!P && 0 !== P) ||
						((Y.modifiers = {
					...
					T &&
						(Y.opacity = (t) =>
							H[t].sd < 0 ? 0 : 0 < H[t].sd ? H[t].a.opacity : "+=0"),
					q.length)
				) {
					O = Q.utils.distribute(O);
					var i = c.slice(q.length);
					Y.stagger = (t, e) =>
						O(~q.indexOf(e) ? c.indexOf(H[t].swap.t) : t, e, i);
				}
				if (
					(it.forEach((t) => r[t] && R.eventCallback(t, r[t], r[t + "Params"])),
					C && c.length)
				)
					for (s in ((W = _(Y, rt)),
				...
				(c.length || j.length || G.length) &&
					(I &&
						R.add(() => X(e, I, R._zTime < 0 ? "remove" : "add"), 0) &&
						!E &&
						X(e, I, "add"),
				...
				n && (n.add(R, 0), tt._final.push(() => ha(H, !x))),
					(o = R.duration()),
					R.call(() => {
						var t = R.time() >= o;
			...
			k &&
				(S = H.filter((t) => !t.sd && !t.a.isVisible && t.b.isVisible).map(
					(t) => t.a.element,
				)),
			tt
			...
			return (
				F || !m
					...
					: ((p = (h = (function _getInverseGlobalMatrix(t) {
						...
						(b = Math.sqrt(p.a ** 2 + p.b ** 2)),
						(w = Math.sqrt(p.c ** 2 + p.d ** 2) * Math.cos(s * N)),
						r &&
							((r = I(r)[0]),
							(u = Q.getProperty(r)),
		...
		ut =
			(((n = FlipState.prototype).update = function update(t) {
				return (
					(this.elementStates = this.targets.map(
						(t) => new ht(t, this.props, this.simple),
					)),
					Da(this),
					this.interrupt(t),
			...
			(n.interrupt = function interrupt(n) {
				...
				this.targets.forEach((t) => {
					var e = t._flip,
						i = lt(e, n ? 0 : 1);
					n && i && a.indexOf(e) < 0 && e.add(() => this.updateVisibility()),
						i && a.push(e);
				}),
		...
		ht =
			...
			(q.update = function update(e, i) {
				...
				(s.uncache = 1),
					...
					(this.width = l ? l.width : P(a("width", "px"), 0.04)),
					(this.height = l ? l.height : P(a("height", "px"), 0.04)),
					e &&
	...
	var U,
		K =
			(((U = FlipBatch.prototype).add = function add(e) {
				var t = this.actions.filter((t) => t.vars === e);
				return t.length
					? t[0]
				...
				return (
					...
					this.actions.forEach((t) => {
						t.vars.getState &&
							...
							e && t.states.forEach((t) => this.state.add(t));
					}),
					(r = n),
					(tt = t),
					this.killConflicts(),
			...
			(U.setState = function setState() {
				return (
					this.actions.forEach(
						(t) => (t.targets = t.vars.setState && t.vars.setState(t)),
					),
					this
				);
			}),
			(U.killConflicts = function killConflicts(e) {
				return (
					...
					this._kill.forEach((t) => t.interrupt(e)),
					this
				);
			}),
	...
	var pt =
		...
		(Flip.killFlipsOf = function killFlipsOf(t, e) {
			(t instanceof ut ? t.targets : I(t)).forEach(
				(t) => t && lt(t._flip, !1 !== e ? 1 : 2),
			);
		}),
