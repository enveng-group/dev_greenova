    ...
    def clean(self):
        ...
        if recurring:
            if not cleaned_data.get("recurring_frequency"):
                self.add_error(
                    "recurring_frequency",
                    "Recurring frequency is required for recurring obligations.")
            if not cleaned_data.get("recurring_status"):
                self.add_error(
                    "recurring_status",
                    "Recurring status is required for recurring obligations.")
        ...
        if inspection:
            if not cleaned_data.get("inspection_frequency"):
                self.add_error(
                    ...
                    "Inspection frequency is required when inspection is enabled.")
            if not cleaned_data.get("site_or_desktop"):
                self.add_error(
                    "site_or_desktop",
                    "Site or desktop is required when inspection is enabled.")
