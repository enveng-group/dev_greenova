...
class ObligationTests(TestCase):
    def test_obligation_navigation_links(self) -> None:
        ...
        mechanism = EnvironmentalMechanism.objects.create(
            name="NavTest Mechanism",
            project=project,
        )
        User.objects.create_user(username="navuser", password="testpass")
        ...
        response = self.client.get(
            f"/obligations/summary/?status=overdue&mechanism_id={mechanism.id}",
        )
        self.assertContains(response, obligation.obligation_number)
    ...
    def test_obligation_creation(self) -> None:
        assert self.obligation.name == "Test Obligation"
        assert self.obligation.description == "Test Description"

    def test_obligation_str(self) -> None:
        assert str(self.obligation) == "Test Obligation"

    def test_obligation_update(self) -> None:
        self.obligation.name = "Updated Obligation"
        self.obligation.save()
        assert self.obligation.name == "Updated Obligation"

    def test_obligation_deletion(self) -> None:
