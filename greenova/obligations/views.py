    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            if not mechanism_id:
                ...
                # Get projects where user is a member using the ProjectMembership model
                # directly
                project_ids = []
...
class ObligationCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            try:
                primary_responsibility = Responsibility.objects.get(
                    name=responsibility_str)
...
def obligation_list_popup(request):
    ...
    if status == "overdue":
        obligations = obligations.filter(
            action_due_date__lt=today).exclude(
            status="completed")
    else:
        obligations = obligations.filter(status=status)
    print(obligations)
