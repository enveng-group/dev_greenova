    ...
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        ...
        try:
            ...
            def user_can_edit_obligation(obligation: Obligation, user: Any) -> bool:
                ...
                """
                return user.has_perm("obligations.change_obligation", obligation)
...
class ObligationDetailView(
    ...
    def user_has_role(self, roles: list[str]) -> bool:
        ...
        """
        return self.request.user.has_perm("obligations.view_obligation", self.object)
...
class ObligationUpdateView(
    ...
    def dispatch(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        if not request.user.has_perm("obligations.change_obligation", self.object):
            messages.error(
                request,
...
class ObligationDeleteView(
    ...
    def post(self, request: HttpRequest, *args: Any, **kwargs: Any) -> JsonResponse:
        ...
        try:
            self.object = self.get_object()
            if not request.user.has_perm("obligations.delete_obligation", self.object):
                return JsonResponse(
                    {
                        "status": "error",
...
def export_obligation(request: HttpRequest, obligation_number: str) -> HttpResponse:
    ...
    if not user.has_perm("obligations.view_obligation", obligation):
        messages.error(request, "You do not have permission to export this obligation.")
        return redirect("dashboard:home")
...
def export_all_obligations(request: HttpRequest) -> HttpResponse:
    ...
    """
    user = request.user
    obligations = get_objects_for_user(
        user,
        "obligations.view_obligation",
        Obligation.objects.all(),
    )
    serializer = ObligationCollectionProtoSerializer(instances=list(obligations))
    data = serializer.data()
    if not data:
