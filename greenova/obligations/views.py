...
class ObligationSummaryView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['mechanisms'] = EnvironmentalMechanism.objects.filter(
                project_id=project_id
            )
            context['phases'] = (
...
class ObligationUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        try:
            ...
            if (
                old_mechanism
                and old_mechanism != obligation.primary_environmental_mechanism
            ):
                if old_mechanism:
...
class ObligationDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            return JsonResponse(
                {'status': 'error', 'message': f'Error deleting obligation: {str(e)}'},
                status=400,
            )
