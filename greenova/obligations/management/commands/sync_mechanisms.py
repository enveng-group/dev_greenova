...
class Command(BaseCommand):
    ...
    def validate_obligations(self) -> bool:
        ...
        try:
            with transaction.atomic():
                ...
                if not self.validate_obligations():
                    msg = "Invalid obligation statuses found"
            ...
            logger.exception("Error syncing mechanisms: %s", str(e))
            self.stdout.write(
                self.style.ERROR(f"Error syncing mechanisms: {e!s}"),
            )
            raise
