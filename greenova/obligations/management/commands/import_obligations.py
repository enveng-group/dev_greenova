...
class ObligationData(TypedDict, total=False):
    ...
    recurring_forecasted_date: Any | None  # Date object
    inspection: bool
...
class Command(BaseCommand):
    ...
    CSV_FIELD_MAPPING: dict[str, str] = {
        ...
        "recurring__forecasted__date": "recurring_forecasted_date",
    ...
    def parse_date_safely(self, date_str: str | None) -> Any | None:
        ...
        try:
            ...
            if date_str.isdigit() and len(date_str) <= 5:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Ignoring possible Excel serial date: {date_str}"), )
                return None

            return parse_date(date_str)
        except Exception as e:
            self.stdout.write(
                self.style.WARNING(f"Failed to parse date '{date_str}': {e!s}"),
            )
            return None
    ...
    def process_row(self, row: dict[str, Any], project: Project) -> ObligationData:
        ...
        recurring_forecasted_date = self.parse_date_safely(
            row.get("recurring__forecasted__date"))
        ...
        non_conformance_comments = self.clean_text(
            row.get("non_conformance__comments", ""))
        ...
        obligation_data: ObligationData = {
            ...
            "recurring_forecasted_date": recurring_forecasted_date,
    ...
    def validate_obligation_data(
            self, obligation_data: ObligationData) -> tuple[bool, str]:
        ...
        if not obligation_data.get("obligation"):
            return False, f"Missing obligation text for {
                obligation_data.get('obligation_number')}"

        # Additional validation can be added here

        return True, ""

    def create_or_update_obligation(self,
                                    obligation_data: ObligationData,
                                    force_update: bool = False,
                                    verbose: bool = False,
                                    ) -> tuple[Obligation | bool | None,
                                               str]:
        ...
        try:
            if existing_obligation:
                ...
                for key, value in obligation_data.items():
                    if (
                        key not in {"obligation_number", "created_at"}
                ...
                if verbose:
                    self.stdout.write(
                        f'Updated fields for {obligation_number}: {
                            ", ".join(
                                obligation_data.keys())}')
            ...
            # This prevents errors with fields that might be in the CSV but not in the
            # model
            model_fields = [field.name for field in Obligation._meta.get_fields()]
            filtered_data = {
                k: v for k,
                v in obligation_data.items() if k in model_fields}
            ...
            if verbose:
                self.stdout.write(
                    f'Created with fields: {
                        ", ".join(
                            filtered_data.keys())}')

            return new_obligation, f"Created obligation {obligation_number}"
        except Exception as e:
            return None, f"Failed to save obligation: {e!s}"
        ...
        with open(file_path, encoding="utf-8-sig") as csvfile:
            ...
            try:
                ...
                for row_num, row in enumerate(
                    ...
                    # Skip empty rows
                    if not any(value.strip() if isinstance(value, str)
                               else value for value in row.values()):
                        if verbose:
                    ...
                    try:
                        ...
                        logger.exception(f"Error processing row {row_num}: {e!s}")
                ...
                logger.exception(f"CSV parsing error: {e!s}")
    ...
    def _process_row(self, row, row_num, default_project, options):
        ...
        if verbose:
            ...
            self.stdout.write(
                f"Processing obligation {obligation_number} (row {row_num})")

        # Create or update the obligation
        if options.get("dry_run"):
            return "created", f"Would import: {
                obligation_data.get(
                    'obligation_number', 'unknown')} - {
                obligation_data.get(
                    'obligation', '')[
                    :50]}..."
        ...
        if isinstance(result, Obligation):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        try:
            ...
            if options.get("dry_run"):
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully created {created} obligations, updated {updated}, skipped {skipped} in {
                            duration:.2f} seconds", ), )
        finally:
            # Reconnect signals
            ...
            if not options.get("dry_run"):
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Failed to update mechanism counts: {e!s}"),
                    )
