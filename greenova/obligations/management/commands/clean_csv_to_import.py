...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        try:
            ...
            self.stderr.write(
                self.style.ERROR(f"Error parsing CSV file: {e!s}"),
            )
        except OSError as e:
            self.stderr.write(
                self.style.ERROR(f"File I/O error: {e!s}"),
            )
        except ValueError as e:
            self.stderr.write(
                self.style.ERROR(f"Value error: {e!s}"),
            )
        except Exception as e:  # pylint: disable=broad-except
            # This is intentionally broad as a last resort for unexpected errors
            self.stderr.write(
                self.style.ERROR(f"Unexpected error cleaning CSV: {e!s}"),
            )
    ...
    def _map_columns(self, df):
        ...
        column_mapping = {
            ...
            "Recurring Forecasted Date": "recurring__forecasted__date",
    ...
    def _clean_project_phases(self, df):
        ...
        if "project_phase" in df.columns:
            ...
            df["project_phase"] = df["project_phase"].astype(str).apply(
                ...
                else x if (pd.notna(x) and x not in {"nan", "NULL", ""} and x in valid_phases)
                else None,
            )
        return df
    ...
    def _clean_site_desktop_values(self, df):
        ...
        if "site_or__desktop" in df.columns:
            df["site_or__desktop"] = df["site_or__desktop"].astype(str).apply(
                ...
                else None if pd.isna(x) or x in {"nan", "NULL", ""}
                else x,
            )
        return df
