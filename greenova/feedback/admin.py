...
class BugReportAdmin(admin.ModelAdmin):
    ...
    def _add_help_text_to_fields(self, form) -> None:
    ...
    def mark_as_rejected(self, request, queryset) -> None:
        queryset.update(status="rejected")

    @admin.action(
        description="Mark selected reports as in progress",
    )
    def mark_as_in_progress(self, request, queryset) -> None:
        queryset.update(status="in_progress")

    @admin.action(
        description="Mark selected reports as resolved",
    )
    def mark_as_resolved(self, request, queryset) -> None:
        queryset.update(status="resolved")

    @admin.action(
        description="Mark selected reports as closed",
    )
    def mark_as_closed(self, request, queryset) -> None:
