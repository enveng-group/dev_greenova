export const enum ObligationStatus {
...
export const encodeObligationStatus: { [key: string]: number } = {
  STATUS_UNKNOWN: 0,
...
export const decodeObligationStatus: { [key: number]: ObligationStatus } = {
  0: ObligationStatus.STATUS_UNKNOWN,
  1: ObligationStatus.STATUS_NOT_STARTED,
  2: ObligationStatus.STATUS_IN_PROGRESS,
  3: ObligationStatus.STATUS_COMPLETED,
  4: ObligationStatus.STATUS_OVERDUE,
};
...
export function encodeObligationInsight(message: ObligationInsight): Uint8Array {
  let bb = popByteBuffer();
  _encodeObligationInsight(message, bb);
  return toUint8Array(bb);
}

function _encodeObligationInsight(message: ObligationInsight, bb: ByteBuffer): void {
...
export function decodeObligationInsight(binary: Uint8Array): ObligationInsight {
  return _decodeObligationInsight(wrapByteBuffer(binary));
}

function _decodeObligationInsight(bb: ByteBuffer): ObligationInsight {
  let message: ObligationInsight = {} as any;
...
export interface ObligationInsightResponse {
  mechanism_id?: number;
  status?: string;
  status_key?: string;
  count?: number;
  total_count?: number;
  obligations?: ObligationInsight[];
  error?: string;
}

export function encodeObligationInsightResponse(message: ObligationInsightResponse): Uint8Array {
  let bb = popByteBuffer();
  _encodeObligationInsightResponse(message, bb);
  return toUint8Array(bb);
}

function _encodeObligationInsightResponse(message: ObligationInsightResponse, bb: ByteBuffer): void {
  // optional int32 mechanism_id = 1;
  let $mechanism_id = message.mechanism_id;
  ...
  let $status_key = message.status_key;
  ...
  let $total_count = message.total_count;
...
export function decodeObligationInsightResponse(binary: Uint8Array): ObligationInsightResponse {
  return _decodeObligationInsightResponse(wrapByteBuffer(binary));
}

function _decodeObligationInsightResponse(bb: ByteBuffer): ObligationInsightResponse {
  let message: ObligationInsightResponse = {} as any;
...
export function encodeChartSegment(message: ChartSegment): Uint8Array {
  let bb = popByteBuffer();
  _encodeChartSegment(message, bb);
  return toUint8Array(bb);
}

function _encodeChartSegment(message: ChartSegment, bb: ByteBuffer): void {
...
export function decodeChartSegment(binary: Uint8Array): ChartSegment {
  return _decodeChartSegment(wrapByteBuffer(binary));
}

function _decodeChartSegment(bb: ByteBuffer): ChartSegment {
  let message: ChartSegment = {} as any;
...
function _encodeChartData(message: ChartData, bb: ByteBuffer): void {
  ...
  let array$segments = message.segments;
  if (array$segments !== undefined) {
    for (let value of array$segments) {
      writeVarint32(bb, 10);
      let nested = popByteBuffer();
  ...
  let $mechanism_name = message.mechanism_name;
...
function _decodeChartData(bb: ByteBuffer): ChartData {
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      case 1: {
        let limit = pushTemporaryLength(bb);
        let values = message.segments || (message.segments = []);
...
export function encodeChartResponse(message: ChartResponse): Uint8Array {
  let bb = popByteBuffer();
  _encodeChartResponse(message, bb);
  return toUint8Array(bb);
}

function _encodeChartResponse(message: ChartResponse, bb: ByteBuffer): void {
  // repeated ChartData charts = 1;
  let array$charts = message.charts;
  if (array$charts !== undefined) {
    for (let value of array$charts) {
      writeVarint32(bb, 10);
      let nested = popByteBuffer();
...
export function decodeChartResponse(binary: Uint8Array): ChartResponse {
  return _decodeChartResponse(wrapByteBuffer(binary));
}

function _decodeChartResponse(bb: ByteBuffer): ChartResponse {
  let message: ChartResponse = {} as any;
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      case 1: {
        let limit = pushTemporaryLength(bb);
        let values = message.charts || (message.charts = []);
