...
describe("Landing Protobuf Bridge", () => {
    it("fetchLandingPageContent calls WASM decode", async () => {
        ...
        window.decodeLandingPageContentWasm = jest.fn().mockResolvedValue(mockContent);
    ...
    it("submitNewsletterSignup encodes and decodes via WASM", async () => {
        window.encodeNewsletterSignupRequestWasm = jest.fn().mockResolvedValue(new Uint8Array([1, 2, 3]));
        window.decodeNewsletterSignupResponseWasm = jest.fn().mockResolvedValue({ success: true, message: "ok" });
        global.fetch = jest.fn().mockResolvedValue({
            arrayBuffer: () => Promise.resolve(new ArrayBuffer(8)),
        });
