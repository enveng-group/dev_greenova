...
- [Charting and Data Visualization Guidelines](#charting-and-data-visualization-guidelines)
  ...
  - [Example: Protobuf Serialization with as-proto and protobufjs (client-side) and protoc (server-side\_](#example-protobuf-serialization-with-as-proto-and-protobufjs-client-side-and-protoc-server-side%5C_)
...
- When both static and interactive charts are needed, use plotly sdk python module as middleware to bridge between matplotlib server-side and plotly.js client-side.
- Always use **protobuf3** and **as-proto and protobufjs (client-side) and protoc (server-side\_** for data serialization and
...
## Example: Protobuf Serialization with as-proto and protobufjs (client-side) and protoc (server-side\_
...
- `proto_utils.py`: Utilities for protobuf and as-proto and protobufjs (client-side) and protoc (server-side\_
  serialization/deserialization.
...
- Use Protobuf3 and as-proto and protobufjs (client-side) and protoc (server-side\_ for all new API endpoints and data serialization tasks.
...
application. These variables must be used in Bootstrap configuration, custom
