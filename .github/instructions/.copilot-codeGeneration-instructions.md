...
- [Charting and Data Visualization Guidelines](#charting-and-data-visualization-guidelines)
  ...
  - [Example: Template Partials](#example-template-partials)
...
1. **Template Structure**:
   ...
   <main class="container">
     ...
     {% include "components/form.jinja" with form=form %} {% block subcontent %}{% endblock subcontent %}
   </main>
   {% endblock content %}
   ```
...
class ObligationListView(LoginRequiredMixin, ListView):
    ...
    paginate_by = 20
    template_name = "obligations/obligation_list.jinja"
...
- Minimize SASS classes; use django-bootstrap5 for styling
  -new development
- Follow technology priority order strictly:
  1. Plain HTML (always start here)
  2. SASS (using django-bootstrap5 as the primary framework)
...
- Use Jinja2 templates (.jinja extension) for new development.
...
- All templates use Jinja2 syntax with `.jinja` extension. DTL (django-html) is not used in this project.
- Configure Django to use only the Jinja2 template engine.
- Ensure all template tags use Jinja2 syntax:
  - `{% url 'name' %}` → `{{ url('name') }}`
  - `{{ variable.strftime('%Y-%m-%d') }}` for dates
  - Remove all `{% load %}` directives
  - `{% trans %}` → `{{ _() }}`
  - `{% include %}` paths reference `.jinja` files
...
- Always use these variables for all new SASS, and Bootstrap theme
  extensions.
