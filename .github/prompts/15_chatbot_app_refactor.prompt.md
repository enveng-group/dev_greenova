...
- [GitHub Copilot Prompt Template for Chatbot App Refactoring](#github-copilot-prompt-template-for-chatbot-app-refactoring)
  - [Goal](#goal)
  - [Context](#context)
...
# GitHub Copilot Prompt Template for Chatbot App Refactoring

## Goal

Refactor the `chatbot` app to fully comply with Greenova's latest project guidelines. This involves ensuring Python code adheres to type safety and documentation standards, templates use Django-Html with `django-bootstrap5`, `django-hyperscript`, and `django-htmx` for dynamic content, and Protocol Buffer integration for chat message serialization.

## Context

The `chatbot` app provides conversational AI functionality for the Greenova application, including chat conversations, message handling, and training data management. All templates must extend the global base from the `core` app and include navigation/sidebar from the new `navigation` and `sidebar` apps.
...
- **Current State**: The `chatbot` app (located at `/workspaces/greenova/greenova/chatbot/`) contains legacy code from the deprecated release that needs modernization.
- **Key Files**: `models.py`, `views.py`, `forms.py`, `proto_utils.py`, `serializers.py`, templates in `templates/chatbot/`, and static files.
...
- **django-guardian**: Use for object-level permissions for conversation access.
- **bleach**: Use for sanitizing user input and HTML content in chat messages.
...
2. **Protobuf3 Integration**
   ...
   - Update chat message serialization to use Protocol Buffers for efficient real-time communication.
3. **Template Refactoring**
   ...
   - Implement real-time chat updates using django-htmx with proper CSRF protection.
4. **Testing & Linting**
...
5. **Permissions & Security**
   ...
   - Use bleach for sanitizing user input and HTML content in chat messages.
   - Implement proper access controls for conversation privacy.
6. **Admin & Middleware**
...
- Implement proper conversation threading and message ordering.
- Use the new core modules for any shared logic or utilities.
- Ensure real-time chat functionality works with django-htmx for seamless user experience.
- All code, templates, and documentation must pass all pre-commit checks and project standards.
- Follow accessibility guidelines for chat interfaces (ARIA labels, screen reader support).
...
- Chat functionality works seamlessly with real-time updates using django-htmx.
- All conversation data is properly secured with object-level permissions.
