...
- [GitHub Copilot Prompt Template for Procedures App Refactoring](#github-copilot-prompt-template-for-procedures-app-refactoring)
  - [Goal](#goal)
  - [Context](#context)
...
# GitHub Copilot Prompt Template for Procedures App Refactoring

## Goal

Refactor the `procedures` app from its legacy state to fully comply with Greenova's latest project guidelines. This includes migrating to `django-bootstrap5`, `django-hyperscript`/`django-htmx`, ensuring Python code quality (type safety, docs, logging), integrating Protobuf3 for data handling, and making all templates and code pass pre-commit checks.

## Context

The `procedures` app manages documented procedures and standard operating procedures (SOPs) related to environmental management tasks, obligations, and mechanisms. This app supports standardized workflows across the **Obligation Management**, **Project Management**, and **Auditing** business processes by providing documented procedures that ensure consistent implementation.

**Business Process Integration:**

- **Procedure Documentation**: Manages SOPs and documented procedures for environmental tasks
...
- **Integrates with**: `obligations` (procedure-obligation relationships), `projects` (procedure implementation), `mechanisms` (procedural controls), `audits` (procedure verification)
- **Serves**: All apps by providing standardized procedures and documentation
...
- **Current State**: The `procedures` app (located at `/workspaces/greenova/greenova/procedures/`) is a legacy app. It may contain outdated JavaScript, styling, and data handling practices.
- **Key Files to Refactor**: `models.py`, `views.py`, `forms.py`, `admin.py`, `urls.py`, `serializers.py`, `proto_utils.py`, `templates/procedures/`, static files in `procedures/static/procedures/`.
...
3. **Protobuf Integration**:
   ...
   - For server-side, compile the required Python stubs (`*_pb2.py`) from the centralized proto directory into the `procedures` app (and any other app that needs them).
   - For client-side, compile JS/TS stubs using `protobufjs` or `as-proto` from the centralized proto directory into `procedures/static/procedures/js/proto/` (or equivalent for each app).
...
4. **Forms and Data Display**:
   ...
   - If displaying lists of procedures, use `django-tables2` styled with Bootstrap 5 and enhanced with HTMX for pagination/sorting.
   - Use `django-autocomplete-light` for any large select fields (e.g., linking procedures to obligations or projects).
5. **Static Files**:
   - Organize SCSS in `static/procedures/scss/` and compile to `static/procedures/css/`.
   - Remove unused or legacy assets.
   - Set up AssemblyScript in `procedures/static/procedures/as/` only if complex client-side logic is essential and cannot be achieved with HTMX/Hyperscript.
6. **Testing & Linting**:
...
7. **Documentation**:
   - Create/update RST documentation for the `procedures` app.

## Sources

- `/workspaces/greenova/greenova/procedures/`
...
- Copilot will refactor the `procedures` app, focusing on modernizing the frontend stack, ensuring Python code quality, and integrating Protobuf for data.
...
- [ ] Protobuf3 integration is complete (`procedures.proto`, `serializers.py`, `proto_utils.py`).
...
- Begin by analyzing existing `models.py` and defining `procedures.proto`.
